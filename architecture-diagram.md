...
graph TD
    ...
    subgraph "ServiceNow Platform (x_eco_)"
        Complaint[(x_eco_complaint)]:::snApp
        Facility[(x_eco_facility)]:::snApp
        Inspection[(x_eco_inspection)]:::snApp
        Finding[(x_eco_finding)]:::snApp
        LegalCase[(x_eco_legal_case)]:::snApp
        AgentLog[(x_eco_agent_log)]:::snApp
...
* **Core Tables**: 
  * `x_eco_complaint` (Intake)
  * `x_eco_inspection` & `x_eco_finding` (Fieldwork)
  * `x_eco_legal_case` (Enforcement)
  * `x_eco_facility` (Compliance Registry)
  * `x_eco_agent_log` (Immutable Audit Trail)
