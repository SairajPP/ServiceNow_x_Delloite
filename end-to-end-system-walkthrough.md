...
Upon submitting the form, a new record is created in the `x_snc_ecosentine_0_complaint` table. The citizen is immediately shown a confirmation page with a unique tracking number (e.g., `ES-20260731-0042`). 
...
As soon as the `x_snc_ecosentine_0_complaint` record is created, the system must decide how urgent this incident is. Normally, a human dispatcher would read the report. In EcoSentinel, this is handled by the **Severity Fusion Agent**.
...
Within seconds, the FastAPI backend sends a `PATCH` request back to ServiceNow using the `x_snc_ecosentine_0.integration_user` account. It updates the complaint with the **AI Severity**, a confidence score, and a human-readable **AI Rationale**.

To ensure total auditability, the backend also `POST`s:
* An **Environmental Snapshot** (`x_snc_ecosentine_0_env_snapshot`) freezing the exact AQI and weather data used at that moment.
* An **Agent Decision Log** (`x_snc_ecosentine_0_agent_decisi`) recording exactly what the AI was fed and what it concluded.
...
**Automated Dispatch:**
Based on the facility's geographic zone (e.g., "North"), Flow Designer (`FL-03`) automatically creates an **Inspection record** (`x_snc_ecosentine_0_inspection`) and assigns it to the `EcoSentinel — North Zone Inspectors` group. 
...
3. While walking the site, they use the app to create **Inspection Findings** (`x_snc_ecosentine_0_finding`). They snap photos of the smokestack and type rough, shorthand notes (e.g., "Burner valve broken. Heavy SO2 smell. Filter bypassed.").
...
Because the inspection was closed with `violation_confirmed = true`, Flow Designer (`FL-05`) automatically generates a **Legal Case** (`x_snc_ecosentine_0_legal_case`), assigning it to the `EcoSentinel — Legal Prosecution Team`.
...
Once the legal case is marked as "Resolved," Flow `FL-11` (Legal Case Resolution Sync) automatically updates the original citizen's `x_snc_ecosentine_0_complaint` to "Closed" and sends the citizen a **Final Resolution Notice** email, thanking them for keeping their community safe.
...
How do we trust the AI? Administrators have access to the AI Control Tower dashboard, which is built on top of the immutable `x_snc_ecosentine_0_agent_decisi` table. Every single time an AI agent makes a decision—whether it's the external FastAPI Severity Fusion or the native Now Assist Report generator—a permanent record is logged here. Admins can audit confidence scores, execution times, and exact prompt inputs/outputs.
...
* **Inspectors** (`x_snc_ecosentine_0.inspector`) can only see inspections assigned to them via the Now Mobile app. They cannot alter the AI's severity or view legal proceedings.
* **Compliance Officers** (`x_snc_ecosentine_0.officer`) triage the complaints and manage facility risk.
* **Legal Handlers** (`x_snc_ecosentine_0.legal_handler`) exclusively manage the penalties and prosecutions.
* **Integration Users** (`x_snc_ecosentine_0.integration_user`) are restricted to API-only access to securely handle the FastAPI webhook payloads.
