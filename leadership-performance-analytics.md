...
* **Access Roles**: `x_snc_ecosentine_0.executive` (Read-only access), `x_snc_ecosentine_0.admin` (Full CRUD/config)  
...
* **Calculation**: Count of `x_snc_ecosentine_0_complaint` records created within the selected time window (Weekly/Monthly).
* **Source Table**: Complaint (`x_snc_ecosentine_0_complaint`)
...
* **Calculation**: Count of open inspections (`x_snc_ecosentine_0_inspection` where `state` < 6) where `sys_created_on` is older than 5 days.
* **Source Table**: Inspection (`x_snc_ecosentine_0_inspection`)
...
* **Source Table**: Facility (`x_snc_ecosentine_0_facility`)
...
* **Source Table**: Legal Case (`x_snc_ecosentine_0_legal_case`)
...
* **Calculation**: `(Number of breached task_sla records / Total task_sla records) * 100` for task_sla records linked to `x_snc_ecosentine_0_inspection`.
...
|---|---|---|---|
| **KPI-01 / W1** | Active Complaints | `x_snc_ecosentine_0_complaint` | `state` < `8` (Closed) |
| **KPI-02 / W5** | Intake Trends | `x_snc_ecosentine_0_complaint` | Grouped by `incident_category`, graphed against `sys_created_on` (trend: daily/weekly) |
| **KPI-03** | Time-to-Assignment | `x_snc_ecosentine_0_complaint` | Average of: (`assigned_to` pop timestamp - `opened_at`) |
| **KPI-04** | Resolution Time | `x_snc_ecosentine_0_complaint` | Average of: (`closed_at` - `opened_at`) where `state` = `8` |
| **KPI-05 / W2** | Inspection Backlog | `x_snc_ecosentine_0_inspection` | `state` < `6` AND `sys_created_on` < `javascript:gs.daysAgo(5)` |
| **KPI-06 / W7** | Risk Profile | `x_snc_ecosentine_0_facility` | Grouped by `risk_tier` (Low/Standard/Elevated/Critical) |
| **KPI-07 / W6** | Legal Throughput | `x_snc_ecosentine_0_legal_case` | Count of records created (`sys_created_on`) vs closed (`closed_at` / state `6`) |
| **KPI-08 / W3** | SLA Breach Rate | `task_sla` | `task_sla.has_breached = true` where `task.sys_class_name = x_snc_ecosentine_0_inspection` |
| **KPI-09 / W8** | Top Risk Facilities | `x_snc_ecosentine_0_facility` | `risk_score` $\ge$ `80` (Order by `risk_score` DESC) |
